...
def resample(x,y,x_new,y_min=None,y_max=None):
    ...
    if x_new[0] <= x[0]:
         ...
         yy.append(0.)        # set y to 0 outside the given x range

    xx.extend(x)
    yy.extend(y)

    if x_new[-1] >= x[-1]:
         # First creation of a new point equal to zero, with the same step as before
...
def lambda_to_nu(wavelength):
    ...
    """
    return cc.c_light_m_s / (wavelength * 1.e-10)
...
def nu_to_lambda(frequency):
    ...
    """
    return 1.e-10 * cc.c_light_m_s / frequency
...
def flambda_to_fJy(wavelength, flambda):
    ...
    # wavelength goes with the Flambda in erg/s/cm2/A).
    fJy = 1e+23 * 1e-10 * flambda * wavelength * wavelength / cc.c_light_m_s
...
def angles_conversion(angle,unit1,unit2):
    ...
    if unit1 not in ['deg','rad','arcmin','arcsec']:
         raise ValueError('incorrect units for unit1\nCorrect units are "deg","rad","arcmin" and "arcsec"')             
    if unit2 not in ['deg','rad','arcmin','arcsec']:
         raise ValueError('incorrect units for unit2\nCorrect units are "deg","rad","arcmin" and "arcsec"')
...
def mean_efficiency_passband(info_dict,passband):
    ...
    """
    w=np.where((info_dict['wavelength_ang'] > info_dict['Passband_cuton']) & (info_dict['wavelength_ang']<info_dict['Passband_cutoff']))
    mean_trans_passband = np.mean(passband[w])
    return mean_trans_passband

#-----------------------------------------------------------------------------------------
def fun_trapz(x,y,dx=None):
   """ Compute the trapeze integration using np.dot instead of np.trapz. It runs about 2 times faster """
   if dx == None: dx=np.diff(x)
