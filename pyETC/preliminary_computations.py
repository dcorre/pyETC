...
def pixel_scale(info_dict):
    ...
    #pixscale = info_dict['FoV_1axis'] / (info_dict['cameras'][info_dict['channel']]['Nphotocell_X'] / info_dict['binning_X'])*60
    pixscale_X = info_dict['FoV_axis1'] / (info_dict['cameras'][info_dict['channel']]['Nphotocell_X'] / info_dict['binning_X'])*60
    pixscale_Y = info_dict['FoV_axis2'] / (info_dict['cameras'][info_dict['channel']]['Nphotocell_Y'] / info_dict['binning_Y'])*60
...
def Focal_length_eff(info_dict):
    """ Computes the effective focal length give the plate scale and the pixel size
    Parameters
    ----------
    ...
    """
    #F_eff = 1./ (info_dict['pixelScale'] * np.pi/(3600*180) / info_dict['pixelSize'])
...
def field_of_view_tel(info_dict,unit='arcmin'):
    ...
    fov_axis1= 2 * np.arctan ( info_dict['cameras'][info_dict['channel']]['Nphotocell_X'] * info_dict['cameras'][info_dict['channel']]['Photocell_SizeX'] / info_dict['foc_len'] / 2.) * 180. / np.pi
    fov_axis2= 2 * np.arctan ( info_dict['cameras'][info_dict['channel']]['Nphotocell_Y'] * info_dict['cameras'][info_dict['channel']]['Photocell_SizeY'] / info_dict['foc_len'] / 2.) * 180. / np.pi
...
def obstruction(info_dict):
    ...
    info_dict['obstruction'] = (info_dict['M2_factor']*info_dict['D_M2'])**2. / info_dict['D_M1']**2.
...
def A_tel(info_dict):
    """
    Compute the aperture area of the telescope taking into account the obscuration
    ...
    """
    exposure_area =  1e4 * np.pi * ((info_dict['D_M1'])/2.)**2. * (1-info_dict['obstruction'])  # cm2
...
def instrument_background(info_dict):
   """ Compute the instrument background emission in function of night temperature
   Parameters
   -----------
   info_dict: dictionary
   ...
   try: 
       data=ascii.read('%s/local_conditions/instrument_background/%s.dat' % (info_dict['MainDirectory'],info_dict['telescope']), "r")
       if info_dict['filter_band'][0] in data.colnames:
           ...
           inst_bg = f(info_dict['Temp_night'])      # e-/s/photocells
           inst_bg *= info_dict['binning_X']  * info_dict['binning_Y'] # e-/s/pixel      
       else:
           inst_bg = 0  # e-/s/pixel
   except:
       #print ('No file found for computing the instrument background of %s. Set to 0' % info_dict['telescope'])
       inst_bg = 0  # e-/s/pixel
...
def system_response(info_dict):
    ...
    if info_dict['detailed_trans'] == 1:
         ...
         sys_eff =  info_dict['Trans_optics_total'] * filter_trans *info_dict['camera_efficiency']
...
def FWHM(info_dict):
    ...
    if FWHM_type == 'seeing':
         ...
         fwhm_tot = np.sqrt(info_dict['seeing_los_arcsec']**2. + fwhm_inst**2.)

    elif FWHM_type == 'diffraction':
         # Diameter of the first mirror in (cm)
         D1 = info_dict['D_M1']*1e2
         fwhm_diffraction =1.22 * info_dict['wavelength_ang']/D1*1e-8*206265
         # Add instrumental psf
         fwhm_tot = np.sqrt(fwhm_diffraction**2. + fwhm_inst**2.)
         ...
         fwhm_tot = np.sqrt(info_dict['seeing_los_arcsec']**2. + fwhm_diffraction**2.)

    elif FWHM_type == 'prf':
         # intrinsinc instrumental psf fwhm_int given as an input
         # pixel response function estimated as a gaussian with the plate scale as the pixel width
...
def PSF_2D_gaussian(sigx,sigy,x,y):
    ...
    psf = 1. / (2.*np.pi * sigx *sigy) * np.exp(-1./2*(x**2./sigx**2. +y**2./sigy**2.))
    return psf
...
def Normalisation_factor(info_dict,central_pix):
    """ Computes the normaliation factor. It corresponds to the fraction of the flux we want to keep. Number in the range[0,1]
    Parameters
    ----------
    info_dict: dictionnary
    ...
    """
    if info_dict['psf_type'] == 'gaussian':
         if central_pix: #computation of the gaussian fraction covered by the brightest pixel
              # Max flux at the center of the pixel
              pX=info_dict['pixelScale_X']    # arcsec
              pY=info_dict['pixelScale_Y']    # arcsec
              a=pX/2.
              b=pY/2.
              sigma = info_dict['FWHM_tot'] / (2.*np.sqrt(2.*np.log(2.)))
              
              norm_factor = dblquad(lambda x, y: PSF_2D_gaussian(sigma,sigma,x,y), -b, b, lambda x: -a*np.sqrt(1-x**2./b**2.), lambda x: a*np.sqrt(1-x**2./b**2.))[0]

         else:   # Flux fraction in the area of radius npix

              # semi X axis
              a = info_dict['radius_aperture_phot']  # arcsec
              siga = info_dict['FWHM_tot'] / (2.*np.sqrt(2. * np.log(2.)))    # (arcsec)
              # semi Y axis
              b = a
              sigb = siga   # arcsec
 
              norm_factor = dblquad(lambda x, y: PSF_2D_gaussian(siga,sigb,x,y), -b, b, lambda x: -a*np.sqrt(1-x**2./b**2.), lambda x: a*np.sqrt(1-x**2./b**2.))[0]
         ...
         if central_pix: #computation of the gaussian fraction covered by the brightest pixel
              ...
              alpha = info_dict['FWHM_tot'] / (2.*np.sqrt(2.**(1./beta)-1.))

              norm_factor = dblquad(lambda x, y: PSF_Moffat(alpha,beta,x,y), -b, b, lambda x: -a*np.sqrt(1-x**2./b**2.), lambda x: a*np.sqrt(1-x**2./b**2.))[0]
...
def nb_pixels(info_dict):
    ...
    R_source = info_dict['spatial_binning'] / 2. #np.ceil(np.pi * (phot_dict['radius_mult_fwhm'] * Fwhm_psf/2.)**2. / (pixsize1*pixsize2))
    
    # side length of the image is a square
    side_length = 2. * R_source
    ...
    if info_dict['PSF_position_on_ccd'] == 'nogrid':
        npix=np.ceil(np.pi* info_dict['radius_aperture_phot']**2/(info_dict['pixelScale_X']*info_dict['pixelScale_Y']))
    ...
    #if info_dict['PSF_position_on_ccd'] == 'nogrid': info_dict['npix']=npix
    #elif info_dict['PSF_position_on_ccd'] == 'center': info_dict['npix']=npix2
    #elif info_dict['PSF_position_on_ccd'] == 'corner': info_dict['npix']=npix3
    #elif info_dict['PSF_position_on_ccd'] == 'mean': info_dict['npix']=(npix2+npix3)/2
...
def factor_images_averaged(info_dict):
    """ Computes the factor mutliplying the variance noise in order to obtain an unbiaised variance in the case of estimating the background noise from several images
    Parameters
    -----------
    info_dict: dictionary
    ...
    factor_averaged: float
                     mutltiplying factor for getting an unbiaised variance when estimating it with several images  
...
def dead_time(info_dict):
    ...
    t_dead = info_dict['T_dithering'] #max(t_dithering(), info_dict['readouttime'])
